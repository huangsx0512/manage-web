package com.zimmur.platform.manage.web.shiro.session;
...
import com.zimmur.platform.manage.web.shiro.CustomShiroSessionDAO;
import com.zimmur.platform.manage.web.shiro.UserOnlineBo;
import com.zimmur.platform.manage.web.util.StringUtils;
...
public class CustomSessionManager {
	private final static Logger LOGGER = LoggerFactory.getLogger(CustomSessionManager.class);
	/**
	 * session status 
	 */
	public static final String SESSION_STATUS ="zimmur-online-status";
	@Autowired
	...
	private UserOnlineBo getSessionBo(Session session){
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			if(null != obj && obj instanceof Account){
				//存储session + user 综合信息
				UserOnlineBo userBo = new UserOnlineBo((Account)obj);
				//最后一次和系统交互的时间
	...
	public Map<String, Object> changeSessionStatus(Boolean status,
		...
		try {
			...
			LOGGER.error(String.format("改变Session状态错误，sessionId[%s]", sessionIds));
			map.put("status", 500);
